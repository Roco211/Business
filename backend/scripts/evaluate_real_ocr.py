...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Evaluate the configured OCR gateway with a local image file.",
    )
    parser.add_argument(
        "image_path",
        help="Path to a local receipt image to evaluate.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OcrProviderError, FileNotFoundError, OSError) as exc:
