...
import { PendingStockInConfirmationCard } from "./PendingStockInConfirmationCard";
...
function buildConfirmation(
  ...
  return {
    confirmation_id: "conf_stock_in_1",
    session_id: "sess_1",
    task_run_id: "task_1",
    confirmation_type: "stock-in",
    status: "pending",
    fields: {
      summary: "Please confirm stock-in.",
      transcript: "Stock in transcript",
      draft_fields: {
...
describe("PendingStockInConfirmationCard", () => {
  ...
  beforeEach(() => {
    ...
    approveConfirmation.mockResolvedValue({ data: { confirmation_id: "conf_stock_in_1", status: "approved" } });
    rejectConfirmation.mockResolvedValue({ data: { confirmation_id: "conf_stock_in_1", status: "rejected" } });
  ...
  it("syncs editable fields with refreshed confirmation draft fields on rerender", async () => {
    ...
    const { rerender } = render(
      <PendingStockInConfirmationCard confirmation={buildConfirmation()} onResolved={onResolved} />,
    );
    ...
    fireEvent.press(screen.getByTestId("confirm-approve-conf_stock_in_1"));

    await waitFor(() => {
      expect(approveConfirmation).toHaveBeenCalledWith("conf_stock_in_1", {
        item_name: "Sprite 330ml",
        quantity: 5,
        unit: "bottle",
        price: 77,
